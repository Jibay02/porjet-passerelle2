<?php 
   ...
   // Connection BDD
    ...
    // editionMode paramétré sur zéro de base
    // $editionMode = 0;

    // Verification si l'article existe pour mettre mode edition sur 1 et afficher les articles à modifier
    // if(isset($_GET['edit']) AND !empty($_GET['edit'])){
    //     $editionMode = 1;
    //     $boris = htmlspecialchars($_GET['edit']);
        
    //     $edit_article = $bdd->prepare('SELECT * FROM blog WHERE id= ?');
    //     $edit_article->execute([$boris]);

    //     if($edit_article -> rowCount() == 1) {
    //        $edited_article = $edit_article->fetch();
    //     }
    // }
    ...
    if(isset($_POST['title']) && isset($_POST['content'])){
        if (!empty($_POST['title']) && !empty($_POST['content'])){
            ...
            $content     = htmlspecialchars($_POST['content']);
                ...
                // On ajoute le titre et l'article ds la BDD
                $req = $bdd->prepare('INSERT INTO blog (title, content) VALUES(?,?)');
                $req->execute([$title, $content]);
                header('location: ArticlesView.php?success=1&message=Votre article à bien été envoyé. Merci à très bientôt');
                exit();
            }
    else{
            // Affichage de l'erreur
            header('location: ArticlesView.php?error=1&message=**Veuillez remplir tous les champs.**');
            exit();
        }
    }
?>
...
<body>
    ...
    <div class="connexion float-end pe-3 pt-3">
        ...
        <div class="Formulaire container">
                        ...
                        <form action="ArticlesView.php" method="post" class=" text-white">
                            <p> Envie de poster un article ? <br> C'est par ici:</p>
                            <input type="text" name="title" id="title" class="form-control-sm opacity-75" placeholder="Titre" /><br><br>       
                            <textarea type="text" rows="3" name="content" id="content" class="form-control opacity-75 w-100 " placeholder="Ecrivez votre article"></textarea><br><br>
